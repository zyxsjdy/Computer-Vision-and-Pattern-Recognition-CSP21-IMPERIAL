...
for k = 1:24
    ...
    y = s*x + in;
    plot(x ,y ,'LineWidth',2,'Color','y'); hold on
end
for k = 28:length(lines)
    lk = [lines(k).point1; lines(k).point2];
    slope = @(line) (line(2,2) - line(1,2))/(line(2,1) - line(1,1));
    intercept = @(line,m) line(1,2) - m*line(1,1);
    s = slope(lk);
...
yline(833.841, 'LineWidth', 4, 'Color', 'r'); hold on;
plot(1920.18, 833.841, 'Marker','.','MarkerSize',50, 'MarkerEdgeColor','c','MarkerFaceColor','c')
